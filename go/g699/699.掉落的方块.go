...
func fallingSquares(positions [][]int) []int {
	...
	squares := make([]square, 0) //记录起点和终点的高度
	...
	for i := 0; i < length; i++ {
		...
		for _, element := range squares {
			//有交集：取反（最小的比最大的都大||最大的比最小的都小）
			if !(square.x > (element.x+element.side-1) || (square.x+side-1) < element.x) {
				height := element.y + element.side
				if height > square.y {
					square.y = height
				}
			}
		...
		if square.y+square.side > max {
			max = square.y + square.side
		}
		r[i] = max
	}
	return r
